...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        # print(pos_idx.size(), loc_t.size(), priors.size())
        loc_p = loc_data[pos_idx].view(-1, 4)
        loc_t = loc_t[pos_idx].view(-1, 4)
        # loc_g = loc_g[pos_idx].view(-1, 4)
        # print(loc_p.size(), loc_t.size(), loc_g.size())
        # priors = priors.expand(num, -1, -1)
        # priors = priors[pos_idx].view(-1, 4)
        loss_l = F.smooth_l1_loss(loc_p, loc_t, size_average=False)
        # repul_loss = RepulsionLoss(sigma=0.)
        # loss_l_repul = repul_loss(loc_p, loc_t, loc_g, priors)
